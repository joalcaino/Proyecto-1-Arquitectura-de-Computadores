// contador_incremental_8bits.v
// Contador de 8 bits de un solo sentido: suma 1 cada flanco de reloj
// mientras "incrementar"=1; se limpia a 00000000 cuando "limpiar"=1
// (limpiar tiene prioridad). Se usa dentro de debounce.v para medir cuantos
// ciclos de reloj seguidos la señal cruda del boton estuvo distinta de la
// señal ya estabilizada.
// Proyecto 1 - Arquitectura de Computadores.
//
// Reutiliza incrementador_8bits.v, mux2_1.v y dff.v, todos ya verificados.

module contador_incremental_8bits (
    ...
    wire [7:0] Q_mas_1, tras_inc, siguiente;

    incrementador_8bits inc (.A(Q), .resultado(Q_mas_1));
